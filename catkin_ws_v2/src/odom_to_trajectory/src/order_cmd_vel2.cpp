...
int main(int argc, char **argv) {
    // ROS 노드 초기화
    ros::init(argc, argv, "order_cmd_vel_2");
    ...
    while (ros::ok()) {
        ...
        } else if (elapsed_time < (2 * side_length / linear_speed)) {
            move_cmd.linear.x = 0; 
            move_cmd.linear.y =  linear_speed;  
        }else if (elapsed_time < (3 * side_length / linear_speed)) {
            move_cmd.linear.x = -linear_speed; 
            move_cmd.linear.y =  0;  
        }else if (elapsed_time < (4 * side_length / linear_speed)) {
            move_cmd.linear.x = 0; 
            move_cmd.linear.y =  -linear_speed;  
        }else{
            // 초기화
